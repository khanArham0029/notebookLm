...
export const useChatMessages = (notebookId?: string) => {
  ...
  const sendMessage = useMutation({
    mutationFn: async (messageData: {
      ...
      const response = await fetch(`${import.meta.env.VITE_FASTAPI_URL || "http://localhost:8000"}/chat/send`, {
        method: "POST",
        headers: {
    ...
    onSuccess: () => {
      ...
      console.log("Message sent to FastAPI successfully")
    },
  })
  ...
  const deleteChatHistory = useMutation({
    mutationFn: async (notebookId: string) => {
      ...
      const { error } = await supabase.from("chat_histories").delete().eq("session_id", notebookId)
